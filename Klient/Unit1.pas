...
application.ProcessMessages;
zadanie:=0;
showmessage('Nie mo¿na pobra dysków poniewa¿'+#10+#13+
              'nawi¹zac po³¹czenie');
listbox1.Clear;
listbox2.Clear;
edit1.Text:='';
...
application.ProcessMessages;
cpower.Host:='';
showmessage('Brak dostêpu do dysków');
zadanie:=0;
cpower.Active:=false;
pob.Active:=false;
krozmiar.Active:=false;
...
if pobieranie=true then
  ...
  showmessage('Trwa pobieranie pliku,'+#10#13+
            'musisz najpierw zakoñczyc ten proces aby pobra nastêpny plik');
   exit;
  end;
try
pobieranie:=true;
...
if listbox2.ItemIndex <0 then showmessage('Nie wybrano ¿adnego pliku')
  ...
  if fileexists(path)=true then
   ...
   krozmiar.Active:=false;
   pob.Host:='';
   showmessage('Brak dostêpu do pliku sprawdŸ po³¹czenie');
   pobieranie:=false;
   end;
end;
...
if rozsciag=rozmiar then
  ...
  pob.Active:=false;
  pob.Host:='';
  showmessage('Pobieranie ukoñczone');
  edit1.Text:='';
  edit2.Text:='';
  edit3.Text:='';
  timer2.Enabled:=false;
...
roz.Active:=false;
srozmiar.Active:=false;
showmessage('Brak dostêpu do pliku poniewaz nie mozna nawiazac polaczenia');
edit1.Text:='';
edit2.Text:='';
edit3.Text:='';
proc.MaxValue:=0;
...
if rozmiar=0 then
  ...
  timer1.Enabled:=false;
  timer2.Enabled:=false;
  showmessage('Ten plik zawiera 0 bajtów');
  pobieranie:=false;
  pob.Active:=false;
  s.Free;
  deletefile(plsciez1);
...
if rozmiar=s.Size then
  ...
  s.Free;
  pobieranie:=false;
  showmessage('Pobieranie tego pliku zosta³o ju¿ ukoñczone');
  exit;
  end;
...
begin
timer1.Enabled:=false;
Showmessage('Brak dostêpu do pliku lub nie mo¿na nawi¹za po³¹czenia');
...
if listbox3.ItemIndex<0 then
  begin
  showmessage('Nie wybrano komputera do usuniêcia');
  exit;
  end;
...
if fileexists(baza) then
  ...
  else showmessage('Nie mo¿na odnaleŸc pliku z baz¹ danych');
end;
...
showmessage('Wyst¹pil bl¹d spróbuj ponownie uruchomic komputer');
end;
end;
...
if ps=4000 then
  ...
  kat.Active:=false;
  showmessage('Wystapil bl¹d podczas przesylania listy plików');
  exit;
  end;
  end;
end;
...
if tekst[1]='1'then
  ...
  if dlug=0 then
   ...
   listbox2.Items.Text:='';
    ...
    kat.Active:=false;
    showmessage('Wyst¹pil bl¹d podaczas pobierania listy katalogów');
    exit;
    end;
   exit;
   end;
...
if messagebox(handle,'CZy napewno chcesz restartowac program ?','Restart',MB_YESNO)=idYes then
  ...
  begin
  showmessage('Trwa pobieranie pliku,'+#10#13+
            'musisz najpierw zakoñczyc ten proces aby zrestartowac program');
   exit;
  end;
  if edit4.Text='' then
    begin
